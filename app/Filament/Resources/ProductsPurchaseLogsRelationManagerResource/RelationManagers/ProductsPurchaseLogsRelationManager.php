...
class ProductsPurchaseLogsRelationManager extends RelationManager
{
        ...
        return ActivityLog::query()
            ->where(function ($query) use ($purchase) {
                $query->where(function ($q) use ($purchase) {
                    $q->where('table_name', 'products_purchases')
                        ->where('primary_key_value', $purchase->purchase_id);
                })->orWhere(function ($q) use ($purchase) {
                    $q->where('table_name', 'products_purchase_items')
                        ->whereIn('primary_key_value', function ($sub) use ($purchase) {
                            $sub->select('id')
                                ->from('products_purchase_items')
