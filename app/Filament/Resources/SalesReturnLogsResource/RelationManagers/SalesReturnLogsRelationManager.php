...
class SalesReturnLogsRelationManager extends RelationManager
{
    protected static string $relationship = 'activityLogs';

    private function getActivityLogQuery(): Builder
    {
        $salesReturn = $this->getOwnerRecord();

        return ActivityLog::query()
            ->where(function ($query) use ($salesReturn) {
                $query->where(function ($q) use ($salesReturn) {
                    $q->where('table_name', 'sales_returns')
